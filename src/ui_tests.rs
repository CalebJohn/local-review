...
fn test_diff_lines_formatting_only_insert_is_dimmed() {
    ...
    let lines = diff_lines(&dc, None, Some(0), None, 99, &AppMode::Normal, false, None);
...
fn test_diff_lines_semantic_filter_hides_pure_formatting_hunk() {
    ...
    let lines_off = diff_lines(&dc, None, Some(0), None, 99, &AppMode::Normal, false, None);
    assert_eq!(lines_off.len(), 2);

    // With filter on: pure-formatting hunk is hidden
    let lines_on = diff_lines(&dc, None, Some(0), None, 99, &AppMode::Normal, true, None);
...
fn test_diff_lines_semantic_filter_shows_mixed_hunk() {
    ...
    let lines = diff_lines(&dc, None, Some(0), None, 99, &AppMode::Normal, true, None);
    assert_eq!(lines.len(), 5); // header + 4 content lines
}
...
fn test_diff_lines_search_highlights_matching_text() {
    ...
    let lines = diff_lines(&dc, None, Some(0), None, 99, &AppMode::Normal, false, Some(("world", false)));
...
fn test_diff_lines_search_case_insensitive() {
    ...
    let lines = diff_lines(&dc, None, Some(0), None, 99, &AppMode::Normal, false, Some(("hello", false)));
    let content_line = &lines[1];
    let has_yellow = content_line.spans.iter().any(|s|
...
fn test_diff_lines_search_no_match_no_highlight() {
    ...
    let lines = diff_lines(&dc, None, Some(0), None, 99, &AppMode::Normal, false, Some(("xyz", false)));
    let content_line = &lines[1];
...
fn test_diff_lines_search_multiple_matches_in_line() {
    ...
    let lines = diff_lines(&dc, None, Some(0), None, 99, &AppMode::Normal, false, Some(("foo", false)));
    let content_line = &lines[1];
